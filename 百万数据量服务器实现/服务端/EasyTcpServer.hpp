...
public:
	...
	bool OnRun()
	{
		//在CellServer里。onrun只用来读取客户端发送的数据
		while(isRun())
		{
			...
			FD_ZERO(&fdRead);
			//将描述符（socket）加入集合
			//FD_SET(_sock, &fdRead)因为主线程已经有了，再加入就会造成内存不足
			//主线程和分线程分别干不同的事
	...
	size_t getClientCount()//得到客户端现有数量
	{
...
private:
	...
	std::mutex _mutex;
	std::thread* _pThread;//为了便于管理
	INetEvent* _pNetEvent;
public:
	std::atomic_int _recvCount;//由主线程负责加
};
...
public:
	...
	void Start()
	{//遍历线程个数_CellServer_THREAD_COUNT=4
